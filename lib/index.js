...
function taskNotFoundError(taskId) {
	return `task not found: ${taskId} (已过期或从未存在; 用 task_list 查看当前队列; 任务默认保留 ${Math.round(runtimeConfig.taskTtlMs / 6e4)} 分钟)`;
}
...
function registerTools(mcp, ctx) {
	...
	mcp.tool("fs_read", "直接读服务器上的文本文件(不用起 agent, 快且免费)。什么时候用: 想确认某个文件现在的内容/某行代码在不在, 又不想为一个只读操作付一次 agent_run 的代价。适合看配置、日志尾部、源码片段。限制: 只能读 ~/.dsh 与已注册工作区白名单内的路径, 且 .ssh/.env/*token*/*.pem 一律拒绝; 单文件 >8MB 拒绝。返回 {path,totalLines,offset,limit,truncated,content}; 文件大就配合 offset/limit 分段读。要看目录列表用 fs_list, 要只看元数据用 fs_stat, 要改文件用 fs_write(需部署开启)。", {
		...
		try {
			...
			return out(JSON.stringify({ error: `fs_read failed: ${e?.message ?? String(e)} (确认路径在工作区内且不是二进制文件)` }));
		}
	});
	mcp.tool("fs_list", "列服务器上的目录内容(不用起 agent)。什么时候用: ① 不知道项目文件都在哪, 先看一眼 ② fs_read 报 path not found 时确认父目录里到底有什么 ③ 找某个文件的全路径。depth 可递归(默认 1 层, 最大 5)。敏感项(.ssh/.env/*token*/*.pem)会从结果里隐藏。返回 {path,depth,count,truncated,entries:[{name,type,size,mtime}]}(type=dir|file|symlink|other, 上限 1000 条)。拿到文件路径后用 fs_read 读内容。", {
	...
	mcp.tool("fs_stat", "只查文件/目录的元数据, 不读内容(判断\"这个文件存在吗/多大/什么时候改的\")。什么时候用: ① fs_read 之前先探一下文件在不在、多大, 避免浪费一次大读取 ② 比较两个文件的 mtime 看谁更新 ③ 确认某个路径是文件还是目录。与 fs_read 的区别: 这个几乎零成本, 且不存在的路径也返回 exists:false 而不是报错。返回 {exists,path,size,mtime,isDir,isFile}(不存在时只有 exists:false 和 path)。", { path: z.string().describe("绝对路径(可以不存在 —— 不存在返回 exists:false 而非报错)") }, async ({ path }) => {
		try {
			...
			return out(JSON.stringify({ error: `fs_stat failed: ${e?.message ?? String(e)} (路径可能在白名单外; 用 config_get 查 workspaceRoots)` }));
		}
	});
	if (runtimeConfig.enableFsWrite) mcp.tool("fs_write", "直接写文本文件(需部署方开启 enableFsWrite; 未开启时本工具不可见)。什么时候用: 确定要落一个已知内容的文件, 且不想为一次简单写入付 agent_run 的代价(如写临时脚本、落配置、追加日志)。若需要 agent 自己判断该改什么, 请用 agent_run/task_inbox。限制: 只能在 workspaceRoots 白名单内(路径 jail), .ssh/.env/*token*/*.pem 一律拒绝, 单次内容 ≤4MB, 父目录会自动创建。返回 {ok,path,bytes,mode}。改完想核对用 fs_read 读回。", {
		...
		try {
			...
			if (bytes > FS_WRITE_MAX_BYTES) return out(JSON.stringify({ error: `content too large (${bytes} bytes > ${FS_WRITE_MAX_BYTES}); 拆成多次 append 写入` }));
			...
			const canonical = gate.canonical;
			if (m === "create-new") {
				if (await stat(canonical).then(() => true, () => false)) return out(JSON.stringify({ error: `file already exists (mode=create-new): ${canonical} (改用 mode=overwrite 覆盖, 或 mode=append 追加, 或换个新路径)` }));
			}
			...
			return out(JSON.stringify({ error: `fs_write failed: ${e?.message ?? String(e)} (确认父目录可写、路径在白名单内)` }));
		}
	});
	mcp.tool("session_list", "【最先调用】列出所有会话(live+已持久化合并), 用来找会话 id / 标题 / 工作目录 / token 用量。什么时候用: 不知道 sessionId、想续接某个历史会话(拿到 id 后传给 agent_run/task_inbox 的 sessionId)、或想知道最近在哪些目录干过活。不传 cwd = 列出全部(默认); 传 cwd = 只看该工作区。返回 {total,count,truncated,skipped,sessions:[{id,title,cwd,createdAt,updatedAt,messageCount,inputTokens,outputTokens,llmTime,...}]}(默认最多 20 条, 按 updatedAt 倒序)。skipped=读取失败被跳过的会话数(单行失败不影响整表)。拿到 id 后: 看对话用 session_log(sessionId=...), 续接干活用 agent_run(sessionId=...)。", {
		...
		try {
			...
			return out(JSON.stringify({ error: `session_list failed: ${e?.message ?? String(e)} (若持续失败, 先用 cwd 过滤缩小范围, 或改用 session_search 按关键词找会话)` }));
		}
	});
	mcp.tool("session_log", "读某个会话的对话/工具调用记录(已剥离 thinking/reasoning 推理块)。什么时候用: 拿到了 sessionId(session_list 或 agent_run 返回)想复盘这次到底说了什么/调了什么工具/为什么失败, 或者 agent_run 结果里的 changes/verification 不够、要看原始过程。返回 {sessionId,header:{cwd,createdAt,preset},types,totalMatched,shown,truncated,events:[{seq,type,time,...}]}——events 按时间正序, 只含最后 tail 条。不想挑类型就用 preset=「dialog」(只看人机对话)或「tools」(只看工具调用); 要精确控制再用 types。", {
		...
		try {
			...
			return out(JSON.stringify({ error: `session_log failed: ${e?.message ?? String(e)} (确认 sessionId 是否正确: 用 session_list 查看; 该会话可能已被清理)` }));
		}
	});
	mcp.tool("session_stats", "看一个会话的用量与性能统计(不读内容, 只看数)。什么时候用: ① 想知道刚才那次 agent_run 花了多少 token / 多久 ② 对比不同 preset 或不同任务的效率 ③ 排查\"怎么这么慢\"(看 ttft/toolTime/cacheHitRate 哪块占大头)。不传 sessionId = 最近一次 agent_run/task_inbox 的会话(最常用); 传 sessionId = 指定会话的全会话累计。返回 {rounds,steps,llmTime,toolTime,ttft,tokensPerSec,cacheHitRate,inputTokens,outputTokens,cacheReadTokens,cacheWriteTokens,reasoningTokens,source}(时间是秒)。想看具体发生了什么用 session_log。", { sessionId: z.string().optional().describe("会话 id(缺省 = 最近一次 agent_run/task_inbox 的会话; 也可传 session_list 里的任意 id)") }, async ({ sessionId }) => {
		try {
			...
			let source;
			if (!target) {
				if (lastAgentSessionId === void 0) return out(JSON.stringify({ error: "no active agent session yet (本进程还没有跑过任何任务; 先调 agent_run 或 task_inbox, 或显式传 sessionId —— 会话 id 可从 session_list 拿)" }));
	...
	mcp.tool("session_search", "不记得 sessionId, 只记得聊过什么 —— 按关键词跨会话找。什么时候用: ① 想找回\"上次讨论 X 的那个会话\" ② 确认某个决定/方案在历史会话里出现过没有 ③ session_list 条目太多翻不过来。先匹配标题, 未命中再尽力扫内容(每会话 2s 超时, 跳过慢的)。返回 {query,regex,total,count,truncated,content_search,results:[{sessionId,title,cwd,updatedAt,matched 为 title 或 content,snippet?}]}(最多 20 条, 按 updatedAt 倒序)。找到 sessionId 后用 session_log 看细节、或 agent_run(sessionId=...) 续接。注意: 内容匹配是\"尽力而为\", content_search=false 说明本次只搜了标题。", {
		...
		try {
			...
			if (regex) try {
				...
				return out(JSON.stringify({ error: `invalid regex: ${e?.message ?? String(e)} (改写成合法正则, 或设 regex=false 按普通文本搜)` }));
			}
			...
			return out(JSON.stringify({
				...
				next: hits.length > 0 ? "拿到 sessionId 后: 看细节用 session_log(sessionId=...), 续接干活用 agent_run(sessionId=...)" : `没有命中; 可尝试: 调大 limit(当前扫了最近 ${scanned.length} 个)、换更短的关键词、或设 regex=true 用正则; 全部会话列表用 session_list`
			}, null, 2));
		} catch (e) {
			return out(JSON.stringify({ error: `session_search failed: ${e?.message ?? String(e)} (可去掉 regex 或缩小 cwd 再试; 单会话读取超时会被跳过, 属正常)` }));
		}
	});
	...
	mcp.tool("preset_set", "改 agent 的能力组合(preset)。两种范围, 先想清楚要哪种: ① scope=「new-default」(默认, 最常用)= 以后新起的会话都用这个 preset, 立刻生效且尽力写进全局用户默认(重启仍在) ② scope=「session」= 只改某一个已存在会话, 且仅限「还没开始任何 turn 的空白会话」(已跑过任务的会话 preset 已固化, 会明确报错)。什么时候用: 发现默认 preset 工具太少/太多, 想换成 code、minimal 等(合法 id 见 preset_list)。返回 {ok,scope,preset,runtimeDefault,globalDefaultUpdated,note?}。若只想给「某一个任务」换 preset, 不用这里 —— 直接 agent_run(preset=...) 更轻。", {
		...
		try {
			if (kind === "session") {
				...
				if (found.events.some((e) => e?.type === "turn/start")) return out(JSON.stringify({ error: `session ${sessionId} has already started; its agent preset is fixed (only blank sessions can switch) (该会话已跑过任务, preset 已固化; 想换能力请用 agent_run(preset=...) 起新会话, 或用 scope=「new-default」改默认)` }));
				let live;
				try {
				...
				if (live) try {
					...
					return out(JSON.stringify({ error: `preset switch failed: ${e?.message ?? String(e)} (用 preset_list 确认 presetId 合法; 或重启会话后重试)` }));
				}
				let handle;
				try {
					handle = await ctx.agents.resume({
					...
					return out(JSON.stringify({ error: `failed to resume blank session ${sessionId}: ${e?.message ?? String(e)} (该会话可能已被清理; 用 session_list 确认; 或改用 scope=「new-default」只改默认)` }));
				}
				try {
			...
			try {
				...
				return out(JSON.stringify({ error: `unknown preset "${presetId}": ${e?.message ?? String(e)} (用 preset_list 查看合法 id)` }));
			}
	...
	mcp.tool("policy_get", "查某个会话现在到底有多少文件权限(沙箱档位), 以及为什么是这个档。什么时候用: ① agent_run 报\"拒绝写入/权限不足\", 先来这里确认实际档位 ② 想确认 danger-full-access 是否真的没开(安全自查) ③ 查审批策略是 ask 还是 never。不传 sessionId = 只看部署默认档(便宜)。返回 {sessionId,sandboxMode,source(override 或 default),workspaceRoot,approvalPolicy}(source=override 表示该会话有专门的 sandbox/mode 记录)。要改档用 set_policy。", { sessionId: z.string().optional().describe("会话 id(缺省 = 只返回部署默认档; 会话 id 来自 session_list 或 agent_run 结果)") }, async ({ sessionId }) => {
		try {
			...
			return out(JSON.stringify({ error: `policy_get failed: ${e?.message ?? String(e)} (可用无参调用看部署默认; 会话 id 用 session_list 确认)` }));
		}
	});
	mcp.tool("set_policy", "改某个已存在会话的文件权限档(什么时候用: agent 抱怨写不了文件 / 需要临时放宽或收紧权限)。三档: read-only(只读, 最安全)|workspace-write(工作区可写, 默认)|danger-full-access(完全绕过围栏 + bash 解禁, 无审批任意读写 —— 仅限可信环境!)。重要限制: 只有 live 会话能改(会追加一条 sandbox/mode 事件, 下一次受限调用即生效, 重启后靠 replay 保持); 冷会话必须先跑一轮(agent_run/task_inbox 带该 sessionId)让它活起来再改。只影响这一个会话; 想给新任务定档请直接用 agent_run(sandbox=...)。返回 {ok,sessionId,sandboxMode,source(固定为 live)}。改完用 policy_get 核对。", {
		...
		try {
			...
			if (!target?.append) return out(JSON.stringify({ error: `session ${sessionId} is not live; cold/persisted sessions must be resumed first (先跑一轮让它活起来: agent_run(task=..., sessionId=...) 或 task_inbox(task=..., sessionId=...), 之后再调 set_policy; 或者干脆在那一轮里用 sandbox=... 直接指定档位)` }));
			...
			return out(JSON.stringify({ error: `set_policy failed: ${e?.message ?? String(e)} (确认 sessionId 正确且会话是 live 的; 用 session_list 查会话, 用 policy_get 查当前档)` }));
		}
	});
	mcp.tool("approval_list", "看有没有\"卡在等人批准\"的请求(agent 想提权/执行敏感操作时, 会一直挂起等回答)。什么时候用: ① agent_run/task_inbox 迟迟不返回, 怀疑卡在审批上 —— 先调这个 ② status_get 显示 pendingApprovals>0 时。返回 {bridge,count,timeoutMs,approvals:[{approvalId,sessionId,toolName,callId?,reason?,requestedAt,waitedMs}]}。count=0 说明没有待审(任务卡住是别的原因)。有则立刻用 approval_respond(approvalId=..., sessionId=..., outcome=...) 回答 —— 不回答的话会一直挂到 approvalTimeoutMs 超时(超时按拒绝收尾, 绝不自动放行)。", {}, async () => {
		const now = Date.now();
		...
		return out(JSON.stringify({
			...
			approvals,
			next: approvals.length > 0 ? `用 approval_respond(approvalId="${approvals[0].approvalId}", sessionId="${approvals[0].sessionId}", outcome="allowed-once"|"rejected") 回答; 已等 ${Math.round(approvals[0].waitedMs / 1e3)}s, 超时 ${Math.round(runtimeConfig.approvalTimeoutMs / 1e3)}s 后按拒绝收尾` : "当前无待审请求; 若任务仍卡住请用 task_list/status_get 查队列与运行态"
		}, null, 2));
	});
	mcp.tool("approval_respond", "批准或拒绝一个挂起的审批(配合 approval_list 用: 先 list 拿 approvalId, 再 respond)。什么时候用: approval_list 显示 count>0, 或 agent_run/task_inbox 卡住不动。两种结果: outcome=「allowed-once」=只放行这一次(最常用, 不放长期权限); 「rejected」=拒绝该操作, agent 会收到拒绝并自己换路子。返回 {ok,receipt(accepted 或 not-pending),approvalId,sessionId,outcome}(receipt=not-pending 表示你慢了 —— 已被 Web UI 或另一路回答/已超时, 先答者胜)。⚠️ 安全提示: 这个工具等于远程提权按钮, 部署在非 loopback 地址时必须配置 authToken。", {
		...
		try {
			...
			if (entry.sessionId !== sessionId) return out(JSON.stringify({
				ok: false,
				error: `sessionId mismatch: approval ${approvalId} belongs to session ${entry.sessionId} (用属于该审批的 sessionId 重试, 见 approval_list)`
			}));
			...
			return out(JSON.stringify({ error: `approval_respond failed: ${e?.message ?? String(e)} (先 approval_list 刷新审批列表再重试)` }));
		}
	});
	...
	mcp.tool("task_result", "取回 task_inbox 提交的任务当前结果(这是异步链路的第二半: task_inbox 拿 taskId → 用本工具轮询)。什么时候用: task_inbox 返回 taskId 之后; 或 agent_run 场景外想确认某个后台任务好了没。返回 {taskId,status,error,result}(status ∈ queued|running|done|error|cancelled; result 仅 done 时有, 含 changes/verification/leftovers/assistantText/toolCalls 等)。轮询建议: running 时等几秒再问, 别高频空转; status=done 即可停。任务不见了(task not found)通常是已过期(默认保留 10 分钟)或被取消。看队列全貌用 task_list。", { taskId: z.string().describe("task_inbox 返回的 taskId(也可从 task_list 的 tasks[].id 取)") }, async ({ taskId }) => {
		...
		return out(JSON.stringify({
			...
			...done ? { next: item.result?.sessionId ? `任务完成; ${HINT.resumeSession}` : "任务完成" } : item.status === "running" || item.status === "queued" ? { next: `仍在${item.status === "running" ? "执行" : "排队"}; 稍后再次调用本工具取结果(建议间隔数秒); 卡在审批上可先看 approval_list` } : item.status === "error" ? { next: "任务失败; 看 error 字段定位原因 —— 常见是权限不足(用 policy_get 查档位)或会话失效(用 session_list 确认)" } : { next: "任务已取消, 结果已丢弃; 需要的话重新用 task_inbox 提交" }
		}, null, 2));
	});
	mcp.tool("task_list", "看异步任务队列的全貌(有哪些排队/在跑/已完成的)。什么时候用: ① task_result 报 task not found, 来这里确认是不是已过期 ② 不记得 taskId 了, 按标题/cwd 找 ③ 确认没有僵尸任务在跑。与 status_get 的区别: 这里列出每一条任务明细, status_get 只给一个 queueActive 总数。返回 {total,active,count,truncated,tasks:[{id,status,createdAt,finishedAt?,error?,title?,preset?,sandbox?,cwd,sessionId?,hasResult}]}(新任务在前, 最多 100 条; status ∈ queued|running|done|error|cancelled)。取具体结果用 task_result(taskId=...)。", {}, async () => {
		try {
	...
	mcp.tool("task_cancel", "取消一个还在排队或正在跑的异步任务(这是 task_inbox 相对 agent_run 的核心优势)。什么时候用: 发现任务方向错了 / 不想等了 / 要腾出队列名额。行为: queued(还在排队)=直接出队; running(正在跑)=尽力中止(结果丢弃, 但会话保留, 之后还能用那个 sessionId 续接); 已完成/已失败/已取消/不存在=明确报错(不可取消)。返回 {ok,status:\"cancelled\",was,taskId,sessionId?,note}。取消后想看队列现状用 task_list。", { taskId: z.string().describe("task_inbox 返回的 taskId(也可从 task_list 取)") }, async ({ taskId }) => {
		try {
			...
			return out(JSON.stringify({ error: `task_cancel failed: ${e?.message ?? String(e)} (用 task_list 确认 taskId 与状态)` }));
		}
	});
	...
	mcp.tool("attach_session", "把一个会话归组到它的工作区下(纯整理操作, 让 dsh Web UI 的工作区侧栏能正确归类)。什么时候用: 会话出现在\"未分组\"里、或 agent_run 建的会话没自动归到期望的工作区。path 不传 = 用该会话 header 里的 cwd。硬性要求: 目标目录必须真实存在, 且 realpath(header.cwd) 必须与工作区路径精确相等, 否则官方 attachSession 会拒绝(这是官方强校验, 本插件无法绕过)。返回 {sessionId,workspaceId,workspacePath,attached}(attached=false 表示本来就在该工作区下)。本插件只做整理, 不改会话内容。", {
		...
		try {
			...
			return out(JSON.stringify({ error: `attach failed: ${e?.message ?? String(e)} (确认 path 目录真实存在; 且 realpath(会话 cwd) 必须与该目录完全相等 —— 不一致时官方会拒绝)` }));
		}
	});
}
/**
