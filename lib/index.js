...
const callbackSchema = z.object({
	...
	events: z.array(z.enum([
		...
		"error",
		"cancelled"
	])).optional().default(["done", "error"]).describe("订阅哪些终态事件(默认 [\"done\",\"error\"]; \"cancelled\" 需显式订阅, 且该事件不含 result)"),
	...
	timeoutMs: z.number().int().min(1e3).max(3e4).optional().default(5e3).describe("单次投递超时毫秒(默认 5000, 上限 30000)")
});
...
function resolveCallback(raw) {
	...
	const events = Array.isArray(rec.events) && rec.events.length > 0 ? rec.events.filter((e) => e === "done" || e === "error" || e === "cancelled") : ["done", "error"];
	...
	let headers;
	if (rec.headers !== void 0 && rec.headers !== null) {
...
function dispatchTaskCallback(item) {
	...
	if (!cb.events.includes(item.status)) {
		item.notify = {
			state: "skipped",
			attempts: 0,
			notifiedAt: Date.now()
