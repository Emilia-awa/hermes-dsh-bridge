...
async function getAgent(ctx, cwd, sessionId, title, requestPreset) {
	...
	if (existing && (requestPreset === void 0 || existing.preset === requestPreset)) {
	...
	if (requestPreset === void 0 || requestPreset === runtimeConfig.preset) {
...
function registerTools(mcp, ctx) {
	...
	mcp.tool("agent_run", "同步执行任务(改代码/分析/跑命令), 返回结构化结果。可传 sessionId 续接已有会话(长任务分多轮投喂)。", {
		...
		const result = await executeTask(ctx, task, context ?? "", cwd ?? process.cwd(), sessionId, title, preset ? { preset } : void 0);
